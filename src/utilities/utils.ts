...
export const easyTruncateAddress = (address: string) => {
  if (!address) return '';
  return address.substring(0, 4) + '...' + address.substring(address.length - 4, address.length);
};

export function customShiftValue(value: number | string, shift: number, unshift?: boolean) {
...
export function isVaultLoanGreaterThanAllowedAmount(vaultLoan: number, allowedAmount: number) {
...
export function formatCollateralInUSD(collateralAmount: string | undefined, bitcoinValue: string | undefined): string {
  if (
    Number.isNaN(collateralAmount) ||
    collateralAmount === undefined ||
    Number.isNaN(bitcoinValue) ||
  ...
  return new Intl.NumberFormat().format(decimalCollateralAmount.mul(decimalBitcoinValue).toNumber());
}
